...
ConvertToENSGAndProcessMatrix <- function(mat, row_id_type) {
  ...
  if (row_id_type == "mouse_symbol") {
    sym2ensmusg <- read.table("/home/ubuntu/gene_features/resources/symbol2ensmusg.txt", header = F, stringsAsFactors = F, col.names = c("symbol", "ENSMUSG"))
  ...
  if (row_id_type == "human_symbol") {
    sym2ensg <- read.table("/home/ubuntu/gene_features/resources/ensg2symbol.txt", header = F, stringsAsFactors = F, col.names = c("ENSG", "symbol"))
    ### Drop rows that don't exist in column
  ...
  if (row_id_type == "ENSMUSG") {
    ensmusg2ensg <- read.table("/home/ubuntu/gene_features/resources/ensmusg2ensg.txt", header = F, stringsAsFactors = F, col.names = c("ENSMUSG", "ENSG"))
    ### Drop rows that don't exist in column
  ...
  ### Read in annotations
  keep <- read.table("/home/ubuntu/gene_features/resources/gene_annot_jun10.txt", sep = "\t", header = T, stringsAsFactors = F, col.names = c("ENSG", "symbol", "chr", "start", "end", "TSS"))

...
# Setup
dir.create(paste0("/home/ubuntu/gene_features/plots/", name))
dir.create(paste0("/home/ubuntu/gene_features/features/", name))
...
read_sparse_mat <- function(x) {
  mat <- readMM(x) %>%
    Matrix(sparse = TRUE) %>%
    as("dgCMatrix")
  file <- sub("_matrix.mtx.gz", "_genes.tsv.gz", x)
  rows <- data.frame(fread(file, header=F), row.names=1)
  file <- sub("_matrix.mtx.gz", "_barcodes.tsv.gz", x)
...
filelist <- Sys.glob(paste0("/home/ubuntu/data/", name, "/*.mtx.gz"))
...
marker_genes <- c("NGFR", "DAPI", "Adamtsl2", "Igfbp3", "Tagln", "Rgs4", "Il34", "Itgb3", "Loxl1", "Sox4", "Podn", "Rspo3", "Spon2", "DES", "VIM", "Gfap", "Pdgfra", "Pdgfrb", "")
